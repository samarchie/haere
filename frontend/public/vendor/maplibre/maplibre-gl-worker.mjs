...
var H = class {
    ...
    update(e, t, r) {
      for (const t of e) {
        ...
        const e = (this._layers[t.id] = n(t, r));
        (e._featureFilter = x(e.filter, `layers[${t.id}].filter`, r)),
      ...
      const i = R(Object.values(this._layerConfigs), this.keyCache);
      for (const e of i) {
...
var W = class {
  ...
  async parse(e, n, r, i, a) {
    (this.data = e), (this.collisionBoxArray = new t());
    const s = new T(Object.keys(e.layers).sort()),
      c = new w(this.tileID, this.promoteId);
    c.bucketLayerIDs = [];
    const l = {},
      u = {
        featureIndex: c,
    ...
    const p = C(u.glyphDependencies, (e) => Object.keys(e).map(Number));
    for (const e of this.inFlightDependencies) e?.abort();
    this.inFlightDependencies = [];
    ...
    let x = u.dashDependencies,
      S = Promise.resolve({});
    if (Object.keys(x).length) {
      const e = new AbortController();
      this.inFlightDependencies.push(e),
        (S = i.sendAsync({ type: `GDA`, data: { dashes: x } }, e));
    }
    const [E, D, O, k] = await Promise.all([m, _, b, S]),
      A = new U(E),
      j = new o(D, O);
    for (const e in l) {
      const t = l[e];
      t instanceof f
        ...
        : t.hasDependencies &&
          (t instanceof te || t instanceof ne || t instanceof y) &&
          (G(t.layers, this.zoom, r),
          t.addFeatures(u, this.tileID.canonical, j.patternPositions, k));
    }
    return {
...
function G(e, t, n) {
  const r = new O(t);
  for (const t of e) t.recalculate(r, n);
}
var K = class {
    constructor() {
  ...
  J = class {
    ...
    loadGeometry() {
      return this.pointsArray.map((e) => e.map((e) => new m(e.x, e.y)));
    }
  },
  Y = class {
    constructor(e, t, n) {
...
function ae(e, t, n) {
  ...
  for (let t = 0; t < e.length; t++) {
    let n = e.feature(t),
      c = n.loadGeometry();
    for (const e of c)
      for (const t of e) (t.x = t.x * i - a), (t.y = t.y * i - o);
...
var oe = class {
    ...
    loadVectorTile(e, t) {
      try {
        return {
          vectorTile: e.encoding === `mlt` ? new j(t) : new D(new v(t)),
          rawData: t,
        };
      } catch (n) {
        ...
        throw (
          ((a += i
            ...
            : `got error: ${re(n).message}`),
          Error(a))
        );
      }
    }
    async loadTile(e) {
      const { uid: t, overzoomParameters: n } = e;
      n && (e.request = n.overzoomRequest);
      const r = this._startRequestTiming(e),
        a = new W(e);
      this.tileState.startLoading(t, a);
      const o = new AbortController();
      a.abort = o;
      try {
        const s = await i(e.request, o);
        if (e.etag && e.etag === s.etag)
          return (
            this.tileState.finishLoading(t), this._getEtagUnmodifiedResult(s, r)
          );
        const c = this.loadVectorTile(e, s.data);
        ...
        const d = this._getExpiryData(s),
          f = this._finishRequestTiming(r);
        (a.vectorTile = l), this.tileState.markLoaded(t, a);
        const p = { rawData: u, cacheControl: d, resourceTiming: f };
        return (
          this.tileState.setParsing(t, p), await this._parseWorkerTile(a, e)
        );
      } catch (e) {
        throw (
          (this.tileState.finishLoading(t), this.tileState.markLoaded(t, a), e)
        );
      }
    }
    _getEtagUnmodifiedResult(e, t) {
      const n = this._getExpiryData(e),
        r = this._finishRequestTiming(t);
      return S({ etagUnmodified: !0 }, n, r);
    }
    async _parseWorkerTile(e, t) {
      ...
      if (n) {
        const { rawData: i, cacheControl: a, resourceTiming: o } = n,
          s = t.overzoomParameters ? `mvt` : t.encoding;
        (r = S({ rawTileData: i.slice(0), encoding: s }, r, a, o)),
    ...
    _getOverzoomTile(e, t) {
      const { tileID: n, source: r, overzoomParameters: i } = e,
        { maxZoomTileID: a } = i,
      ...
      for (const e in l) {
        const r = t.layers[e];
        if (!r) continue;
        const i = ae(r, a, n.canonical);
        i.length > 0 && c.addLayer(i);
      }
      const u = { vectorTile: c, rawData: M(c).buffer };
  ...
  X = class {
    ...
    async loadTile(e) {
      const {
        ...
        u = r.height + 2,
        d = ee(r)
          ? new a({ width: l, height: u }, await E(r, -1, -1, l, u))
          : r,
        f = new B(t, d, n, i, o, s, c);
  ...
  se = class {
    ...
    loadVectorTile(e) {
      ...
      const { z: t, x: n, y: r } = e.tileID.canonical,
        i = this._geoJSONIndex.getTile(t, n, r);
      if (!i) return null;
      const a = new F(i.features, { version: 2, extent: c });
      return { vectorTile: a, rawData: M(a, V).buffer };
    }
    async loadTile(e) {
      const { uid: t } = e,
        n = new W(e);
      ...
      try {
        const r = this.loadVectorTile(e);
        if (!r) return null;
        const { vectorTile: i, rawData: a } = r;
    ...
    async _parseWorkerTile(e, t) {
      ...
      if (n) {
        const { rawData: t } = n;
        (r = S({ rawTileData: t.slice(0), encoding: `mvt` }, r)),
          this.tileState.removeParsing(e.uid);
      }
      return r;
    }
    ...
    async loadData(e) {
      ...
      const t = this._startRequestTiming(e);
      ...
      try {
        ...
        if ((delete this._pendingRequest, !u(e))) throw e;
        return { abandoned: !0 };
      }
    }
    _startRequestTiming(e) {
    ...
    _filterGeoJSON(e, t, n) {
      ...
      const r = this._getFilterPredicate(t, n);
      return r
        ? {
...
function Z({ geojsonVtOptions: e, clusterProperties: t, source: n }) {
  ...
  return (
    (e.clusterOptions.map = (e) => {
      o.properties = e;
      const t = {};
      for (const e of s) t[e] = r[e].evaluate(a, o);
      return t;
    }),
    (e.clusterOptions.reduce = (e, t) => {
      o.properties = t;
      for (const t of s) (a.accumulated = e[t]), (e[t] = i[t].evaluate(a, o));
    }),
    e.clusterOptions
  );
}
async function Q(e) {
  ...
  if (/^[ \t]*(import|export)\s/m.test(n)) {
    const e = URL.createObjectURL(new Blob([n], { type: `text/javascript` }));
    try {
      await import(e);
    } finally {
...
var $ = class {
  constructor(t) {
    (this.self = t),
      ...
      this.actor.registerMessageHandler(`RS`, async (e, t) => {
        ...
        const n = this.workerSources[e][t.type][t.source];
      ...
      this.actor.registerMessageHandler(`UGS`, async (e, t) => {
        const n = this._getGlobalState(e);
  ...
  async _setImages(e, t) {
    ...
    for (const n in this.workerSources[e]) {
      ...
      for (const e in r) r[e].availableImages = t;
    }
  }
  async _syncRTLPluginState(e, t) {
    return await L.syncState(t, Q);
  }
  ...
  _getWorkerSource(e, t, n) {
    if (
      ...
      switch (t) {
        case `vector`:
          this.workerSources[e][t][n] = new oe(
            r,
